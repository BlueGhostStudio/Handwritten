...
ObjectModel {
    ...
    Component {
        ...
        ColumnLayout {
            ...
            function load(slice) {
                if (canvas.available)
                    canvas.load(sopid, realtime, slice)
                else {
            ...
            Component.onCompleted: {
                canvas.initial(sopid, realtime).then(function () {
                    load(true)
                })
            }
        }
    ...
    function addInboxItem(sopid, from, datetime, realtime) {
        var paperItem = slipOfPaperCmp.createObject (fakeRoot, { from: from, sopid: sopid, datetime: datetime, realtime: realtime })

        append(paperItem)
    }

    function loadInbox(ret) {
        for (var x in ret) {
